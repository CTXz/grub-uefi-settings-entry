...
# Args
if [ ! -z $3 ]; then CONFIG_PATH=$2; fi # Check if custom config path has been passed as argument
if [ ! -z $4 ]; then INSTALL_OUT=$3; fi # Check if userdefined grub custom entries directory has been passed as argument

# Check which update script is used
if [ ! -z $2 ]; then GRUB_MKCONFIG=$1 # Custom upgrade script passed as argument
